...
def load_vgg(sess, vgg_path):
    ...
    return vgg_input_tensor, vgg_keep_prob, vgg_layer3_out, vgg_layer4_out, vgg_layer7_out
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    def transposed_upsample(x, num_classes, depth, strides, init=init):
    ...
    transposed_1 = transposed_upsample(layer_7_1x1, num_classes, 5, 2)
    layer_1 = tf.layers.batch_normalization(transposed_1)
    layer_1 = tf.add(layer_1, layer_4_1x1)

    transposed_2 = transposed_upsample(layer_1, num_classes, 5, 2)
    layer_2 = tf.layers.batch_normalization(transposed_2)
    layer_2 = tf.add(layer_2, layer_3_1x1)

    return transposed_upsample(layer_2, num_classes, 14, 8)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        for image, label in get_batches_fn(batch_size):
            ...
            if step % 5 == 0:
                print("Epoch {}/{}...".format(i+1, epochs), "Training Loss: {:.4f}...".format(loss))
            step += 1
...
def run():
    ...
    with tf.Session() as sess:
        ...
        train_nn(sess, epoch, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
             correct_label, keep_prob, learning_rate)
